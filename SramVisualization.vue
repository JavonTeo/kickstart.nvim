...
<template>
	<div class="sram-container">
		...
		<ErrorMessage v-if="error" :message="error" :show-retry="true" @retry="fetchSramData" />

		<!-- Loading indicator while fetching data -->
		<LoadingSpinner v-if="loading" message="Loading SRAM data..." />

		<!-- Main SRAM grid display -->
		<SramGrid v-else-if="weightData.length > 0" :data="weightData" @cell-hover="showCellDetails"
			@cell-leave="hideCellDetails" :title="title" />

		<!-- No data message with retry option -->
		<ErrorMessage v-else message="No SRAM data available. Click 'Retry' to load data." :show-retry="true"
			@retry="fetchSramData" />

		<!-- Cell tooltip for hover information -->
		<CellTooltip v-if="showTooltip" :data="tooltipData" :position="{ x: tooltipX, y: tooltipY }" />
	</div>
</template>
